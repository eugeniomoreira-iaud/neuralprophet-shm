...
Updated 2026-09-07 · 04:30 UTC. This block is the summary a reader needs to follow the
...
- **Execution machine.** From 2026-09-07 the study runs on a Windows workstation: an AMD Ryzen Threadripper PRO 9995WX with 64 physical cores, simultaneous multithreading disabled, 500 GB of RAM, PyTorch 2.5.1 on the CPU, and NumPy 1.26.4 on OpenBLAS. MKL is excluded by `environment.yml` and must stay excluded: paired with NumPy 1.26.4 the current MKL kills the interpreter on any call into BLAS or LAPACK, with no Python traceback. Every timing recorded in this plan before that date was measured on the ten-core Apple Silicon laptop the study was started on; those figures are kept as the historical baseline that motivated Task 5.4c and are not comparable with anything measured since. Two consequences bind every task that spawns processes. Worker processes are spawned rather than forked, so each one re-imports torch and pays a fixed startup cost of seconds, which makes a large pool a net loss on a loop with few items. And the parallel width that matters is the length of the loop being parallelised rather than the core count: the longest loop in the study is `rolling_nowcast`'s roughly seventy walk-forward origins, and the shortest are three regressor sets and five trend-regularisation candidates. (Machine change, 2026-09-07.)
...
- [x] **Step 2: Sync, execute (sweeps are long; background, timeout 43200 s), copy back, commit** `feat(study05): three monitoring charts at three time scales, attribution and detectability by mechanism`. Fix round 1 (`task-5.4-fixround-addendum.md`, rulings 1–7) committed as 7d1e7f3.

### Task 5.4c (S): Parallel fits — `n_jobs` across origins, folds, candidates, sets and rungs

**Why:** every NeuralProphet fit in this study is small and every library loop over fits is serial, so the notebook leaves nearly the whole machine idle while it runs. Measured on 2026-09-06 on the ten-core laptop the study began on, a full run occupied about 1.5 cores (4 performance, 6 efficiency; torch reported 4 threads and the run sat at 15 % CPU), Movement 3 alone (three sets × about seventy refits) took about 45 minutes, and the whole notebook about two hours, with every fix re-running it. That measurement is the historical motivation for this task. Since 2026-09-07 the study runs on the 64-core Threadripper recorded in the Global Constraints, where the same serial loops leave sixty-odd cores idle, so the case for process-level parallelism is stronger rather than weaker and the ceiling on it has moved from the core count to the length of the loops themselves. Nothing about any individual fit changes.
...
**Interfaces:**
- `_parallel_map(function, items, n_jobs=1)`: when `n_jobs == 1` it returns `[function(item) for item in items]` and joblib is never imported; otherwise `joblib.Parallel(n_jobs=min(n_jobs, len(items)), backend='loky')(joblib.delayed(wrapped)(item) for item in items)`, where `wrapped` calls `torch.set_num_threads(1)` in the worker before `function`. The pool is capped at the number of items deliberately, and the cap earns its keep on this machine rather than the old one: workers are spawned rather than forked, so each surplus worker re-imports torch and contributes nothing, and the short loops here — three regressor sets, five trend-regularisation candidates, a handful of ladder rungs — would otherwise pay to start dozens of processes that immediately go idle. Results come back in the order of `items`; any exception propagates unchanged.
...
- Notebook: `N_JOBS = 32`. Bullet: `1` reproduces the serial run bit for bit; the useful ceiling is the length of the longest loop, about seventy walk-forward origins, rather than the machine's 64 cores, and `_parallel_map` caps the pool at the item count on the shorter loops, so a value above the loop length costs startup and buys nothing; each worker holds its own copy of the frame, about one gigabyte, so 32 workers occupy roughly 32 GB of the machine's 500 GB. The value is a starting point, not a measurement: confirm it against Step 3's per-movement timings and adjust once those exist.

**Steps:**
- [x] **Step 1: Tests (RED).** `TestParallelFits` in `test_model_a.py`: (a) `_parallel_map(double, [1, 2, 3], n_jobs=2) == [2, 4, 6]`, and order is preserved with `n_jobs=3` on a function whose items sleep for decreasing times; (b) `rolling_nowcast` on the synthetic frame the file already uses, `epochs=2`, a `refit_every` that yields at least three origins, once with `n_jobs=1` and once with `n_jobs=2`: `pd.testing.assert_frame_equal(serial, parallel, check_exact=False, rtol=1e-5)` and identical `origin` columns; (c) `sweep_trend_reg` with two candidates and `n_jobs=2` returns the same candidates in the same order as serial, values within `rtol=1e-5`.
- [x] **Step 2: Implement (GREEN).** Run the Global Constraints suites plus `test_model_a.py`, `test_model_b.py`, `test_harmonics.py`, `test_monitor.py`; output pristine. Commit 1: `feat(shmlib): fit the walk-forward origins, folds, candidates, sets and rungs in parallel processes`.
- [x] **Step 3: Notebook and verification run** (done inside 7d1e7f3 with the fix round's shared run; comparison in `task-5.4-fixround-report.md`: every body `.tex` outside Movement 5 identical, CSVs within 5e-4 relative against a baseline made on the laptop, so machine and parallelism are conflated; a same-machine serial control run is recorded there). Add `N_JOBS` and the `n_jobs=N_JOBS` keywords (Movements 2, 2b, 3, and 6 once it exists). Before the run, copy `outputs/GM_*.csv` and `outputs/GM_*_body.tex` to a scratch directory `gm05_before_parallel/`. Run the whole notebook to the scratch as usual; read each cell's wall time from `metadata.execution` in the executed `.ipynb` and total it per movement. Compare every `GM_*.csv` numerically against the copy (a scratch script: same shape and columns, numeric columns `np.allclose(rtol=1e-6, atol=1e-9)`, other columns equal) and every `_body.tex` textually; paste the comparison table (file, maximum relative deviation) and the per-movement timings into `task-5.4c-report.md`. A body `.tex` that differs textually is reported with both versions so the controller re-reads that table's numbers in the report. Copy back, honesty test. Commit 2: `feat(study05): parallel fits across the notebook, results unchanged within tolerance`.
- [x] **Step 4: Failure mode** (exercised: `MemoryError` from a 143 GB worker payload and `OSError 22` from a shared log folder on Drive, both reproduced serially and fixed in 250f5bd). If `n_jobs > 1` fails inside the Jupyter kernel on Windows — where loky always spawns, so every worker re-imports torch and receives the closure through cloudpickle — in a way that `torch.set_num_threads(1)` in the worker or `JOBLIB_START_METHOD` cannot fix, report BLOCKED with the traceback; never fall back to threads silently (the GIL would give no speed-up and hide the failure). Expect one Windows-specific shape of failure that carries no Python traceback at all: a worker that dies in native code surfaces only as joblib's `TerminatedWorkerError` naming an exit code, and an exit code of `0xC0000005` (`-1073741819`) is an access violation inside the worker process, not a logic error in the loop. Reproduce such a case serially with `n_jobs=1` before drawing any conclusion about the parallel path.
