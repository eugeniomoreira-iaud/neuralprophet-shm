...
### Task 2.4c (S implements, O specifies the figure): The coverage chart that replaces Table 1

**Files:**
- Modify: `studies/shmlib/figures.py` (+ `plot_regressor_set_coverage`), `studies/shmlib/tests/test_shmlib.py` (one axes-count test), the notebook's Movement 1 coverage cell.

From the user's revision notes in `report/report05_check.md` (§2, Table 1): the coverage table becomes a chart generated by the notebook, per the provenance rule above.

- [ ] **Step 1: `figures.plot_regressor_set_coverage(coverage, target_label, title='', save_path=None, filename=None)`.** One vertical stacked bar per record — the inclination first, then the three roles of each set grouped by source in the order on-structure, station, ERA5 — with the accepted slots as the solid bar and the filled slots stacked above it in the same identity colour at reduced alpha; the left axis in slots, the right twin axis in per cent of the window's slots (the same total for every bar, so the two axes are one linear scale); role identity colours from `viz.channel_style`; group labels beneath the axis; the legend for accepted/filled below the axes per the graphical guidelines. Test: a small synthetic `coverage` frame with the inclination row and two sets → one primary axes plus its twin (`len(fig.axes) == 2`).
- [ ] **Step 2: Notebook.** In Movement 1's "Coverage and the anatomy of the target's gaps" cell, after the `GM_01` table is written, call the function with `filename='GM_F01b_set_coverage'`; the step's Markdown cell gains one sentence saying the chart is the report's Figure of the coverage. Sync, execute to the scratch directory, verify, copy back.
- [ ] **Step 3: Commit** `feat(study05): the coverage of the inclination and the three regressor sets as a chart`.
...
- [ ] §2 revisions from `report/report05_check.md`: spell out the on-structure (STR), ground-station (GS) and ERA5 sources at their first appearance; replace Table 1 with `\includegraphics{GM_F01b_set_coverage.png}` and a caption in the same voice; rewrite the coverage paragraph so that each sentence states one fact about one source; update every sentence that referred to Table 1.
...
- [ ] Write §9 from `GM_16_body.tex` and `GM_F14_ladder.png`: one paragraph per rung, the verdict sentence per channel (probe and pyranometer), and the caveat that rung 4 uses the probe's future values and is a diagnostic only. Build, honesty test, README status, commit `docs(study05): write what the wall temperature and the pyranometer buy`.
